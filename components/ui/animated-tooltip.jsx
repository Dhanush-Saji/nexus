          ...
          <AnimatePresence mode="popLayout">
            { hoveredIndex === item.userId && (
              <motion.div
                ...
                className="absolute -top-16 left-[-100%] translate-x-[200%] flex text-xs flex-col items-center justify-center rounded-md bg-black z-[99999] shadow-xl px-4 py-2"
              >
                <div className="absolute inset-x-10 z-30 w-[20%] -bottom-px bg-gradient-to-r from-transparent via-emerald-500 to-transparent h-px " />
                <div className="absolute left-10 w-[40%] z-30 -bottom-px bg-gradient-to-r from-transparent via-sky-500 to-transparent h-px " />
                <div className="font-bold text-white relative z-30 text-base">
                {session?.data?.user?.id == item.userId && <span className="mr-2">⭐</span>}
                 {item.name}
                </div>
